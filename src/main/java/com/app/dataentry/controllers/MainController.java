...
public class MainController {
  ...
  public String main(Model model) {
    if (isOperator()) {
      model.addAttribute("count", "Clients entered by you: " +
                                      clientService.operatorCount());
    } else {
      model.addAttribute("count",
                         "Groups in system: " + clientService.groupsCount());
    }
    return "index";
  }
  ...
  public String login(Model model,
                      @RequestParam(value = "error",
                                    required = false) boolean hasError) {
    if (hasError) {
  ...
  public byte[] report(Model model, HttpServletResponse response) {
    ByteArrayOutputStream output =
        reportService.generateReport(clientService.getPages());
  ...
  public String smscode(@ModelAttribute UserDto userDto) {
    ...
    if (user == null || !bCryptPasswordEncoder.matches(userDto.getPassword(),
                                                       user.getPassword())) {
